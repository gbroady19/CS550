...
The goal of this project was to hide games in a user's screen so that they could be played without other people knowing. Once the program is run through terminal, the user has 3 seconds to go to their chosen background window before the program takes a screenshot that will be used as the backgroud of the game. There are to games to choose from, a "sneaky" snake game and a "buried" brick breaker game. This can be chosen through the GUI after launch. The snake game is controlled using 'W' for up, 'M' for down, 'L' for left and 'A' for right. These controls were chosen to make it less obvious that the user is playing a game. For brick breaker the controls are just the arrow keys, left arrow for left, right arrow for right. The mouse object, that is used in both games, changes depends on the user's OS. While playing, the user can minimize and pause the game by pressing esc and resume by reopening the window and pressing any movement key. The user can exit back to the main menu using esc when each game is over. On the main menu, the user can quit by pressing esc. 
