...
#Recursively drawn line based fractal loosely based on Cantor set 
#Sources: 
...
def drawOut(x, y, dx, dy, num, den):
	...
	if dx == 0 and dy == 0:
		return
	#draw an inital starting line in two directions  
...
#Initial calling of function, starting lines are drawn at the center of the image. The initial num and dem were chosen after experiementation to produce the most interetesing looking fractal
...
#Interesting function in ImageOps in PIL that inverts pixels above a certain threshold value. 
...
for Py in range(imgy):
	...
	for Px in range(imgx):
		...
		while (x*x + y*y <= 2) and iteration < max_iteration: 
			...
			iteration += 1 
			x = xtemp
		# Played around with colorspace. Although calculation is simple, using the right combination of ImgX variable, Px variable and the all important iteration variable 
